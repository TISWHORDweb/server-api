    ...
    <body>
        <div class="wrapper" style='width:95%; table-layout: fixed; background: #fff; padding-bottom:60px; font-family: "Plus Jakarta Sans", sans-serif; border: 10px solid #FFF7E6'>
            <table class="main" width="100%">
                ...
                notify(user._id, note.title, note.body, data, true, locate.ip, req.body.device)
            };
            ...
            await MongoroUserModel.updateOne({_id: user._id}, {$set: {ip: add, active: true}}).then(() => {
                res.status(200).json({
...
router.post('/sendchamp', async (req, res) => {
    ...
    const locate = ipapi.location(callback)
